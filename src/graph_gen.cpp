...
{
    std::cout << "Creating Nested Squares Graph ... "
              << "\n";
    ...
    forall_nodes(v, G)
    {
        ...
        forall_nodes(k, G)
        {
            // connect every node with the node of the parent square( ex. node 1 with node 5)
            if (number[k] - number[v] == 4)
            {
                G.new_edge(v, k);
            }
    ...
    std::cout << "Nested Squares Graph is ready for benchmark!"
              << "\n";
    return G;
}

// a graph generator for ring graphs
ugraph ring_graph(int n, int m)
{
    std::cout << "Creating Ring Graph ... "
              << "\n";
    ...
    forall_nodes(v, G)
    {
        ...
        forall_nodes(k, G)
        {
            ...
            // connect every node with the node of the  square except the last with its nest( ex. node 1 with node 2, node 2 with node 3)
            if (number[k] - number[v] == 1)
            {
                G.new_edge(v, k);
            }
            // Connect last node with first
            if (number[v] == n && number[k] == 1)
            {
                G.new_edge(v, k);
            }
        }
    ...
    // return undirected graph G
    std::cout << "Ring Graph is ready for benchmark!"
              << "\n";
    return G;
}
